...
class TuneManager:
    ...
    def auto_tune(self, is_sql_useful):
        ...
        print(
            f"[auto_tune] {is_sql_useful} current index:{self.current_sql_index} total tune threshold:{self.tune_gap}"
        )  # or use logging
    ...
    def immediate_tune(self):
        ...
        # print(sqls)
        # print("-" * 50)
        # for query in [sql["sql"] for sql in sqls]:
        #     print(query)
        # print("-" * 50)
        # self.tuner.init([sql["sql"] for sql in sqls])
    ...
    def update_progress(self, progress: float):
        ...
        self.r.hset(
            "mab:tuning_status",
            mapping={"progress": str(progress), "last_updated": now},
        )

    def complete_tuning(self):
        now = datetime.now().isoformat()
        self.r.hset(
            "mab:tuning_status", mapping={"is_tuning": "false", "last_updated": now}
        )
