...
from db_tools.qplan_pg.index_use import IndexRead, ViewRead, IndexWrite, IndexRead
from bandits.bandit_arm_MV_v1 import BanditArmMV
from db_tools.db_bandit_helper import get_estimated_size_of_index_v1, get_estimated_size_of_mv_v2
# import db_tools.postgres_db as sql_helper

sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))

import db_tools.postgres_db as postgres_db
...
def gen_arms_from_predicates_v2(db:postgres_db, query_obj):
    """
    ...
    for table_name, table_predicates in predicates.items():
        ...
        for col_permutation in col_permutations:
            ...
            if arm_id in bandit_arm_store:
                ...
                size = get_estimated_size_of_index_v1(db, constants.SCHEMA_NAME,
            ...
            if includes:
                ...
                for col_permutation in col_permutations:
                    ...
                    if arm_id_with_include not in bandit_arm_store:
                        size_with_includes = get_estimated_size_of_index_v1(db, constants.SCHEMA_NAME,
                                            table_name, col_permutation + tuple(includes))
...
def gen_frq_table_subsets(query_objs, all_tables, query_properties):
    frq_table_subsets = {}
    total_workload_time = 0
    min_rows = sum([x.table_row_count for x in all_tables.values()]) * 0.1
    for query_index, query_obj in enumerate(query_objs):
        total_workload_time += query_obj.original_running_time
        # print("query_properties[query_index]", query_properties[query_index])
        table_set = list(query_properties[query_index]["predicates"].keys())
        ...
        for i in range(2, len(table_set)+1):
            ...
            for table_subset in table_subsets:
                # print("query_properties[query_index]['joins']", query_properties[query_index]['joins'])
                join_list = query_properties[query_index]['joins']
                if join_list:
                    # TODO here is to find if joined columns belong to tables
                    if can_be_joined(table_subset, join_list):
...
def gen_mv_arms_from_predicates_v3(db, query_obj, tables, frequent_table_subsets, query_properties, include_group_by):
    """
    This method take predicates (a dictionary of lists) as input and creates the generate arms for all possible
    column combinations
    ...
    bandit_arms = {}
    query_id = query_obj.id
    group_by_possible = include_group_by and query_properties[query_id]['payload']

    for table_subset in frequent_table_subsets:
        if set(table_subset).issubset(set(query_properties[query_id]['tables'])) and \
                can_be_joined(table_subset, query_properties[query_id]['joins']):
            group_bys = {False}
            if group_by_possible and set(table_subset) == set(query_properties[query_id]['tables']):
                group_bys.add('True')
            for group_by in group_bys:
                ...
                if arm_id in bandit_arm_store:
                    ...
                    for table_name in table_subset:
                        if table_name in query_properties[query_id]['payload']:
                            arm_payload[table_name] = query_properties[query_id]['payload'][table_name]
                            for payload_column in query_properties[query_id]['payload'][table_name]:
                                if payload_column in tables[table_subset[0]].pk_columns:
                                    columns_pk.append(payload_column)
                                if group_by:
                                    if query_properties[query_id]['payload'][table_name][payload_column] != 'PL':
                                    ...
                                    payload_list.append(f"\t{table_name}.{payload_column} as {table_name}_{payload_column}")
                                if query_properties[query_id]['payload'][table_name][payload_column] == 'EQ':
                                    indexed_columns_equal.append(f"{table_name}_{payload_column}")
                                elif query_properties[query_id]['payload'][table_name][payload_column] in {'GT', 'LT', 'GE', 'LE'}:
                    ...
                    if payload_list:
                        ...
                        join_list = []
                        arm_joins = {}
                        for (table1, table2), join_columns in query_properties[query_id]['joins'].items():
...
def finalizing_mv_arms(db, bandit_arms_mv, query_properties, max_memory):
    ids_to_delete = []
    db.conn.timeout = 60
    for arm_id, arm in bandit_arms_mv.items():
        if arm.memory == -1:
            ...
            size = get_estimated_size_of_mv_v2(db, arm.payload,
...
def get_mv_arm_view_query(arm_id, view_comps):
    view_query = f"CREATE VIEW {arm_id} AS\n"
    view_query += "SELECT\n"
    view_query += ",\n".join(view_comps["payload_list"]) + "\n"

    view_query_from = "FROM " + ", ".join(view_comps["table_subset"]) + "\n"

    if view_comps["join_list"]:
        view_query_from += "WHERE " + "\n  AND ".join(view_comps["join_list"]) + "\n"

    if view_comps["group_by_columns_original"]:
        view_query_from += "GROUP BY " + ", ".join(view_comps["group_by_columns_original"]) + "\n"

    view_query += view_query_from + ";"

    # Count query
    count_query = "SELECT COUNT(1) AS C1\n" + view_query_from
    if view_comps["group_by_columns_original"]:
...
def get_mv_arm_index_query(arm_id, index_comps):
    index_query = f"CREATE UNIQUE INDEX {arm_id}_idx ON {arm_id} (\n"
...
def get_predicate_position(arm, predicate):
    """
...
def get_context_vector_v2(bandit_arm, all_columns, context_size, uniqueness=0, includes=False):
    ...
    if len(bandit_arm.name_encoded_context) > 0:
        ...
        i = 0
        for column_name in all_columns.keys():
        # for table_name in all_columns:
        #     for k in range(len(all_columns[table_name])):
            column_position_in_arm = get_predicate_position(bandit_arm, column_name)
            ...
            elif column_name in bandit_arm.include_cols:
                include_context[i] = 1
            i += 1
...
def get_context_vector_mv_v1(db, bandit_arm, all_columns, number_of_columns, chosen_arms_last_round):
    """
    Return the context vector for a given arm, and set of predicates. Size of the context vector will depend on
    the arm and the set of predicates (for now on predicates)
...
def get_super_arm_context_v1(db, arm, ucb, chosen_arms_last_round, static_context_size, number_of_clusters):
    database_size = db.get_database_size()
...
def get_derived_value_cv_v4(db, bandit_arm_dict, query_obj_list, chosen_arms_last_round,
                            with_includes):
    """
...
def get_view_encode_cv_v1(db, bandit_arm_dict, all_columns, number_of_columns, chosen_arms_last_round):
    """
    ...
    context_vectors = []
    for key, bandit_arm in bandit_arm_dict.items():
        context_vector = get_context_vector_mv_v1(db, bandit_arm, all_columns, number_of_columns,
...
def get_super_bandit_context(db, chosen_arms, chosen_arms_last_round, static_context_size, number_of_clusters):
    original_map = []
    arm_list = []
    context_list = []
    for table, super_arm in chosen_arms.items():
        for key, (arm, arm_id, ucb) in super_arm.items():
            if arm.memory > 0:
                arm_list.append(arm)
                context_list.append(get_super_arm_context_v1(db, arm, ucb, chosen_arms_last_round, static_context_size, number_of_clusters))
...
def get_index_writes(query_plan):
    ...
    """
    if isinstance(query_plan, IndexWrite):
        return query_plan.indices
    return None
