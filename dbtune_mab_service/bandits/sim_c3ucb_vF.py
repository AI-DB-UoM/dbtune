...
from db_tools.db_bandit_helper import (
    bulk_drop,
    create_query_v7,
    hyp_check_config,
)
...
class BanditTuner:
    ...
    def _create_bandits_for_tables(self):
        ...
        for table_name in self.candidate_tables:
            ...
            context_size = (
                self.column_counts[table_name]
                * (1 + constants.CONTEXT_UNIQUENESS + constants.CONTEXT_INCLUDES)
                + constants.STATIC_CONTEXT_SIZE
            )

            # Create oracle and the bandit
            oracle = Oracle(self.max_memory)
            self.bandits_dict[table_name] = bandits.C3UCB(
                context_size,
                self.input_alpha,
                self.input_lambda,
                oracle,
    ...
    def _create_bandits_for_MV(self):
        if self.with_mv:
            ...
            self.bandits_dict[self.mv] = bandits.C3UCB(
                context_size,
                self.input_alpha,
                self.input_lambda,
                OracleMV(self.max_memory),
                self.cluster_id,
            )
            self.cluster_id += 1
    ...
    def _create_super_bandit(self):
        ...
        self.super_bandit = bandits.C3UCB(
            self.cluster_id + self.super_static_context_size,
            self.input_alpha,
            self.input_lambda,
            oracle_s,
        )
        ...
        while current_round < self.rounds:
            ...
            for key, obj in self.query_obj_store.items():
                if current_round == 0:
                    ...
                    if (
                        current_round - obj.last_seen <= constants.QUERY_MEMORY
                        and 0 <= obj.first_seen < current_round
                    ):
                        query_obj_list_past.append(obj)
            ...
            if current_round > 0 and len(query_obj_additions) > 0:
                ...
                for table_name in self.candidate_tables:
                    self.bandits_dict[table_name].workload_change_trigger(
                        workload_change
                    )
                if self.with_mv:
            ...
            if self.with_mv:
                frequent_table_subsets = bandit_helper.gen_frq_table_subsets(
                    query_obj_list_past, self.tables, self.query_properties
                )
            for i in range(len(query_obj_list_past)):
                bandit_arms_tmp = bandit_helper.gen_arms_from_predicates_v2(
                    self.db, query_obj_list_past[i]
                )
                bandit_arms_mv = {}
                # print("with_mv", self.with_mv)
                if self.with_mv:
                    bandit_arms_mv = bandit_helper.gen_mv_arms_from_predicates_v3(
                        self.db,
                        query_obj_list_past[i],
                        self.tables,
                        frequent_table_subsets,
                        self.query_properties,
                        True,
                ...
                for key, index_arm in bandit_arms_tmp.items():
                    ...
                    index_arm.clustered_index_time += query_obj_list_past[
                        i
                    ...
                    for key, index_arm in bandit_arms_mv.items():
                        ...
                        for ta in index_arm.table_names:
                            index_arm.clustered_index_time += (
                                max(query_obj_list_past[i].table_scan_times[ta])
                                if query_obj_list_past[i].table_scan_times[ta]
                                else 0
                            )
                        index_arms[table_name][key].query_ids.add(index_arm.query_id)
                        index_arms[table_name][key].query_ids_backup.add(
                            index_arm.query_id
                        )

            # set the index arms at the bandit
            if self.mv in index_arms:
                bandit_helper.finalizing_mv_arms(
                    self.db, index_arms[self.mv], self.query_properties, self.max_memory
                )
            ...
            for table_name in self.candidate_tables:
                ...
                index_arm_list = list(index_arms_for_table.values())
                logging.info(
                    f"Generated {len(index_arm_list)} arms for table {table_name}"
                )
                ...
                for i in range(len(context_vectors_v1)):
                    context_vectors.append(
                        numpy.array(
                            list(context_vectors_v2[i]) + list(context_vectors_v1[i]),
                            ndmin=2,
                        )
                    )
                # getting the super arm from the bandit
                chosen_arm_ids[table_name] = self.bandits_dict[table_name].select_arm(
                    context_vectors, current_round
                )
                ...
                if chosen_arm_ids[table_name]:
                    for arm_id, ucb in chosen_arm_ids[table_name]:
                        ...
                        chosen_arms[table_name][index_name] = (
                            index_arm_list[arm_id],
                            arm_id,
                            ucb,
                        )
            ...
            if self.with_mv:
                ...
                context_vectors = bandit_helper.get_view_encode_cv_v1(
                    self.db,
                    index_arms_for_table,
                    self.all_columns,
                ...
                chosen_arm_ids[self.mv] = self.bandits_dict[self.mv].select_arm(
                    context_vectors, current_round
                )
                chosen_arms[self.mv] = {}
                if chosen_arm_ids[self.mv]:
                    for arm_id, ucb in chosen_arm_ids[self.mv]:
                        index_name = index_arm_list[arm_id].index_name
                        chosen_arms[self.mv][index_name] = (
                            index_arm_list[arm_id],
                            arm_id,
                            ucb,
                        )

            super_arm_list, super_context, original_map = (
                bandit_helper.get_super_bandit_context(
                    self.db,
                    chosen_arms,
                    chosen_arms_last_round,
            ...
            super_chosen_arm_ids, mv_size_weight, index_size_weight = (
                self.super_bandit.select_super_arm_v2(super_context)
            )
            ...
            if current_round < self.hyp_check_rounds:
                ...
                hyp_query_plans, hyp_cost = hyp_check_config(
                    self.db,
                ...
                hyp_arm_rewards = bandit_helper.calculate_hyp_reward(
                    query_obj_list_current, hyp_query_plans
                )
                end_time_hyp_reward = datetime.datetime.now()
                hyp_cost += (
                    end_time_hyp_reward - start_time_hyp_reward
                ).total_seconds()
            ...
            arm_rewards = bandit_helper.calculate_reward(
                creation_costs, query_obj_list_current, query_plans
            )
            ...
            self.super_bandit.update_super_v3(
                super_chosen_arm_ids,
                arm_rewards,
                useless,
                mv_size_weight,
                index_size_weight,
            ...
            for table_name in self.candidate_tables:
                arm_ids = (
                    super_chosen_per_table[table_name]
                    if (table_name in super_chosen_per_table)
                    else []
                )
                self.bandits_dict[table_name].update(
                    arm_ids, arm_rewards, useless, mv_size_weight, index_size_weight
                )

            if self.with_mv:
                arm_ids = (
                    super_chosen_per_table[self.mv]
                    if (self.mv in super_chosen_per_table)
                    else []
                )
                self.bandits_dict[self.mv].update(
                    arm_ids, arm_rewards, useless, mv_size_weight, index_size_weight
                )
            ...
            recommendation_time = (
                (end_time_round - start_time_round).total_seconds()
                + hyp_cost
                - (end_time_create_query - start_time_create_query).total_seconds()
            )
            logging.info(
                "Recommendation cost: "
                + str(recommendation_time)
                + ", Hyp Component: "
                + str(hyp_cost)
            )
            ...
            print(
                f"current total {current_round}: ",
                total_time,
                ", this round: ",
                total_round_time,
            )
            logging.info(
                f"current total {current_round}: {total_time}, this round: {total_round_time}"
            )
        ...
        for t in range(self.rounds):
            ...
            for key, obj in self.query_obj_store.items():
                if t == 0:
                    ...
                    if (
                        t - obj.last_seen <= constants.QUERY_MEMORY
                        and 0 <= obj.first_seen < t
                    ):
                        query_obj_list_past.append(obj)
            ...
            if t < self.hyp_check_rounds:
                # def hyp_check_config(db, schema_name, arm_list_to_add, queries, file_path):
                hyp_query_plans, _ = hyp_check_config(
                    self.db,
                    constants.SCHEMA_NAME,
                    added_arms,
                    query_obj_list_current,
            ...
            print(
                f"current total {t}: ", total_time, ", this round: ", total_round_time
            )
            logging.info(
                f"current total {t}: {total_time}, this round: {total_round_time}"
            )
