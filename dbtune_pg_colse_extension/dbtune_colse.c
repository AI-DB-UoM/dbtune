...
static size_t
writefunc(void *ptr, size_t size, size_t nmemb, void *userdata)
{
...
static char *
send_query_to_colse(const char *sql_text)
{
    CURL *curl = NULL;
    CURLcode rc;
    CurlString response;
    ...
    if (rc != CURLE_OK)
        elog(WARNING, "[DBTune CoLSE] HTTP request failed: %s", curl_easy_strerror(rc));
    else
...
static void
maybe_send_query(const char *query_text)
{
...
static void
colse_ExecutorStart(QueryDesc *queryDesc, int eflags)
{
...
static void
colse_ProcessUtility(PlannedStmt *pstmt,
...
_PG_init(void)
{
    ...
    DefineCustomStringVariable(
        "dbtune_colse_service_url",
        "The CoLSE estimate service URL.",
        NULL,
        &colse_service_url,
        "http://colse_api:5060/colse/estimate",
        PGC_SIGHUP,
        0,
        NULL, NULL, NULL);

    DefineCustomIntVariable(
        "dbtune_colse_timeout_ms",
...
Datum
dbtune_colse_estimate(PG_FUNCTION_ARGS)
{
